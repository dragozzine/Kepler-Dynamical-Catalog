...
for file in range(len(files)):
    if files[file]:
        print(f"Reading ../data/{input_files[file]}.parquet ...")
        df = pd.read_parquet(f"../data/{input_files[file]}.parquet")
...
if file_type.lower() == "csv":
    ...
    ar_csv.write_csv(table, f"../data/{filename}.csv")
    ...
    combined.to_hdf(f"../data/{filename}.h5", key="kdc")
elif file_type.lower() == "parquet":
    combined.to_parquet(f"../data/{filename}.parquet")
else:
